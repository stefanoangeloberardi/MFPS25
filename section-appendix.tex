...
%08:56 19/06/2024
\begin{proof}(Uniqueness of Typing, Prop. \ref{proposition-left-finite-unique})
...
\begin{proof}(Total terms and Finiteness for Safe-Reductions, Lemma \ref{lem:total_value-finiteness})
\begin{enumerate}
%%%%%%%%%%%%%%%%%%%%%
...
\subsection{The Iterator}\label{subsection-iterator}
%10:43 16/04/2024
\begin{example}[The iterator]\label{example:iterator}
...
\begin{proposition}[$\Iter$ is in $\CTlambda$]
\label{proposition-iterator-in-CT-lambda}
$\iter \in \Reg \cap \WTyped \cap \GTC = \CTlambda$, 
...
\begin{prooftree}
  \AxiomC{}
  \RightLabel{$\var$}
  \UnaryInfC{$\Gamma \vdash a:\N$}
  \AxiomC{}
  \RightLabel{$\var$}  
  \UnaryInfC{$\Gamma, x:\N \vdash f:\N \rightarrow \N$}
  \AxiomC{$\vdots$}
  \noLine
  \UnaryInfC{$\Gamma, \vdash \iter: \vnode6{\N}\rightarrow\N$\quad($\dagger$)}
  \RightLabel{$\weak$}
  \UnaryInfC{$\Gamma, x:\vnode5{\N}\vdash \iter: \N\rightarrow\N$}
  \RightLabel{$\apvar$}    
  \UnaryInfC{$\Gamma, x:\vnode4{\N}\vdash \iter(x): \N$}
  \RightLabel{$\apnotvar$}  
  \BinaryInfC{$\Gamma, x:\vnode3{\N} \vdash f(\iter(x)):\N$}
  \RightLabel{$\lambda$}  
  \UnaryInfC{$\Gamma\vdash \lambda x.f(\iter(x)):\vnode2{\N} \rightarrow \N$}
  \RightLabel{$\cond$}
  \BinaryInfC{$\Gamma \vdash \iter: \vnode1{\N} \rightarrow \N$ \quad ($\dagger$)}
\end{prooftree}
$
...
\subsection{The Interval Map}\label{subsection-interval}
...
%% On the role of the $\weak$-rule, 
%% if we want a graph notation, we add a back edge from the 
%% upper $(\dagger)$ to the bottom $(\dagger)$.
%% The $\weak$-rule is used to remove the variables $x$ and $y$ from the context,
%% since they are no more used in the top $(\dagger)$
%% and do not occur in the bottom $(\dagger)$. 
...
\begin{center}
  ...
  %%   %      \rightarrow\alpha}
  %%   {\infer[\lambda]  
  %%     {\Gamma \vdash \interval:\N,\goldNout\rightarrow\alpha 
  %%       \ \ \ (\golddagger) }
  %%     {\infer[\cond]{\Gamma, x:\N 
  %%         \vdash 
  %%         \cond (\base,  \inductive )
  %%         :\goldNout\rightarrow\alpha \ \ \ (\goldspadesuit) } 
  %%       {\infer[\apvar]{\Gamma, x:\N 
  %%           \vdash 
  %%           \base:\alpha}
  %%         {\infer*{\Gamma, x:\N 
  %%             \vdash 
  %%             \cons(\iter(x)):\alpha \rightarrow\N}{}}
  %%         {\ \ \ \ \ \ }  &
  %%         \infer[\lambda]{\Gamma, x:\N 
  %%           \vdash 
  %%           \lambda y.j : \goldNout\rightarrow\alpha}
  %%               {\infer[\apnotvar]{\Gamma, x:\N, y:\goldNout \vdash
  %%                   j : \alpha}
  %%                 {\infer*
  %%                   {\Gamma, x:\N, y:\N 
  %%                     \vdash \iter(x):\N}{}
  %%                   {\ \ \ \ \ \ }
  %%                   {\infer[\apvar]{\Gamma, x:\N, y:\goldNout 
  %%                       \vdash \interval(\Succ(x))(y):\alpha}
  %%                     {\infer[\apnotvar]{\Gamma, x:\N, y:\N 
  %%                         \vdash \interval(\Succ(x)):\goldNout \rightarrow\alpha}
  %%                       {\infer[\weak]{\Gamma, x:\N, y:\N 
  %%                           \vdash \interval:\N,\goldNout \rightarrow\alpha}
  %%                         {\infer*{\Gamma 
  %%                             \vdash \interval:\N,\goldNout \rightarrow\alpha
  %%                             \ \ \ (\golddagger)}
  %%                           {} } & {\ldots}  }}}
  ...
  \begin{scriptsize}
    \begin{prooftree}
      ...
      \AxiomC{$\vdots$}
      \noLine
      \UnaryInfC{$\Gamma_{x^\N} \vdash \iter(x): \N$}
      \doubleLine
      \UnaryInfC{$\Gamma_{x^\N} \vdash \base_x:\alpha$}
      \AxiomC{$\vdots$}
      \noLine
      \UnaryInfC{$\Gamma_{x^\N} \vdash \iter(x): \N$}
      \AxiomC{$\vdots$}
      \noLine
      \UnaryInfC{$\Gamma \vdash \interval: \N,\vnode8{\N}\rightarrow\alpha$ \hspace{3ex} ($\dagger$)}
      \RightLabel{$\weak$}
      \UnaryInfC{$\Gamma_{x^\N}, y:\N \vdash \interval: \N,\vnode7{\N}\rightarrow\alpha$}
      \AxiomC{$\vdots$}
      \noLine
      \UnaryInfC{$\Gamma_{x^\N}, y:\N \vdash \Succ(x): \N$}
      \RightLabel{$\apnotvar$}
      \BinaryInfC{$\Gamma_{x^\N}, y:\N \vdash \interval(\Succ(x)): \vnode6{\N}\rightarrow\alpha$}
      \RightLabel{$\apvar$}
      \UnaryInfC{$\Gamma_{x^\N},y:\vnode5{\N} \vdash \interval(\Succ(x))(y): \alpha$}
      \doubleLine
      \BinaryInfC{$\Gamma_{x^\N},y:\vnode4{\N} \vdash \inductive_{x,y}: \alpha$}
      \RightLabel{$\lambda$}
      \UnaryInfC{$\Gamma_{x^\N} \vdash \lambda y.\inductive_{x,y}: \vnode3{\N} \rightarrow \alpha$}
      \RightLabel{$\cond$}      
      \BinaryInfC{$\Gamma_{x^\N} \vdash \cond(\base_x,\lambda y.\inductive_{x,y}):\vnode2{\N} \rightarrow \alpha$}
      \UnaryInfC{$\Gamma \vdash \interval:\N,\vnode1{\N}\rightarrow\alpha$ \hspace{3ex} ($\dagger$)}
    \end{prooftree}
    % DRAW EDGES
...
%16:32 30/04/2024
%22:30 03/06/2024
\subsection{The ackermann function (Better code is easier to analyse)}
