...
\begin{Eg}[The sum map]\label{example-sum}
...
For the third example, we simulate lists with one type variable $\alpha$ 
and two variables $\nil:\alpha$ and $\cons:\N \rightarrow \alpha \rightarrow \alpha$. 
We recursively define a notation for lists by $[]=\nil$,
and $[a,\vec{a}] = \cons(a)( [\vec{a}] )$. We add no elimination rules 
for lists, though, only the variables $\nil$ and $\cons$. Elimination rules are not 
required in our example. Recall that $\approx$ denotes having a common reduct.

\begin{Eg}
Let $\Sigma = \{\nil:\alpha, \cons:\N,\alpha \rightarrow \alpha\}$. 
We will define a term $\Interval$ in the context $\Sigma$,
having one argument $f:\N \rightarrow \N$ and three arguments $a,x,y:\N$, such that 
\[
\Interval(f,a,n,m) \  \approx \ [f^n(a), f^{n+1}(a), \ldots, f^{n+m}(a)] \  : \ \alpha
\]
for all numeral $n,m \in \Num$. $\Interval$ will satisfy the following equations
\begin{center}
...
defined in the previous sub-section \ref{subsection-iterator}. 
In particular, $\iter(n) \approx f^n(a) : \N$ for all $n \in \Num$.
We solve the equations above for $\Interval$ with 
$\Interval = \lambda f. \lambda a.\interval$, where 
\[
...
\]
where the base case $\base$ and the inductive case $\inductive$ are
\[
\base 
\ \  = \ \   
[\iter(x)]
