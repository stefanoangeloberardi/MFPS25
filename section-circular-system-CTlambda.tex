...
First, it is a waste of time to recompute the value of $u$ if this value is already
stored in $x$. Second, if we write $\ldots (\redm,n') \ldots$ instead of 
$\ldots (\Suc{m'},n') \ldots$, then we use a variable as argument,
we add more connections and more traces
to our term, and it is more likely that the Global Trace algorithm proves
the Global Trace condition and therefore that the map we defined is total. 
Replacing  $\Suc{m'}$ with $\redm$, when this replacing is sound, 
provides better code which is easier to analyse automatically.
