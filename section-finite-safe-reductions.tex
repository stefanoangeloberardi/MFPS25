...
\begin{lemma}[The $\prec$-order]\label{lemma-prec-order}
  ...
  \item\label{lemma-prec-order-01}
    ...
    for well-typed $t$ of type $\N$ and $t$ is \emph{finite for safe-reductions}.
...
Then, considering a trace taken by $t\in\GTC$, there is an infinitely decreasing sequence for $\prec$,
...
the rightmost path $\pi$ of $\Pi$ is infinite, cyclic, with all nodes 
labeled by the judgment $\vdash t: \N$. The path $\pi$ is 
...
The first claim is shown by induction of the leftmost branch of $t$, 
...
Intuitively, $\Lim(t)$ is the limit of a sequence of $n$-safe-normal forms of $t$ ($n=0,1,2,\ldots$). 
