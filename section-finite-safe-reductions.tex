...
subterms of $t$ in any infinite path $\pi$ of a proof $\Pi :: \Gamma \vdash t : A$ 
...
some \emph{total} trace-compatible assignment  $\rho$ to $\pi$. Then we have the following clauses. 
\begin{enumerate}
\item
...
\[
\vdash f : \N,\redN \rightarrow\N, \ \ \ 
x^\N:\N \vdash \cond(x,f(\Succ x)) : \redN \rightarrow\N, \ \ \ 
x^\N:\N \vdash f(\Succ x) : \redN \rightarrow\N, \ \ \ 
\vdash f : \N,\redN \rightarrow\N, \ \ \ 
\ldots
\]
