...
\item
The free variable $x:A$ in $\Gamma \vdash f(x) : B$ and in 
$\Gamma$ is connected to the free variable $x$ in the premise
$\Gamma \vdash f:A \rightarrow B$, but also to the argument number $n+1$, the 
first unnamed argument of the premise, to which $x$ is sent.

\item
The first unnamed argument $A$ of the premise $\Gamma t:A \rightarrow B$ in the proof of $\Gamma \vdash t(u):B$ is connected to no argument type of 
the conclusion.
...
corresponds to the argument number $k+1$ of the premise. 

\item
If $x^A$ has index $i$ in the conclusion 
$\Gamma \vdash \lambda x^A.t : A \rightarrow B$, 
then $x^A$ is connected to no argument type of the premise, because the 
...
and any argument number $k \ge i+1$ of the conclusion
corresponds to the argument number $k-1$ of the premise. 

\item
The first unnamed argument $\N$ of the conclusion 
$\Gamma \vdash \cond(f,g):\N \rightarrow A$, having number $n+1$, 
correspond to no argument of the premise $\Gamma \vdash f:A$, 
and any argument number $k \ge n+2$ 
of the conclusion corresponds to the argument 
number $k-1$ of the premise for $f$. 
\end{enumerate}
