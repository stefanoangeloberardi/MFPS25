...
We define the following \emph{stationary} connections.
\begin{enumerate}
...
\quad
\end{center}
where the arrow from $\vec{B}$ to $\vec{B}$ means that each lower $B_k\in\vec{B}$ is connected to upper $B_k$. 
\item
...
the $j$-th named argument $x_j^{A_j}$ in the conclusion is connected to the $j$-th named 
argument $x_j^{A_j}$ in the assumption. hoge
\begin{prooftree}
\RightLabel{$\Succ$}
\def\extraVskip{2pt}
\def\ScoreOverhang{0pt}
