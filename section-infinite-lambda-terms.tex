...
the subset $\Reg$ of of $\LAMBDA$ consisting of regular trees, which we call 
\emph{circular terms}, 
...
\item
We write $t \sim_\alpha u$ if there is some \emph{equality-preserving} 
$\alpha$-conversion map $\Phi:v \rightarrow w$ such that $u$ is obtained from $t$
by replacing a single sub-term $v$ of $t$ with $w$.
\end{enumerate}
\end{definition}
...
by replacing the subterm $\lambda y.y$ of $t$ with $\lambda x.x$.
With this definition of $\sim_\alpha$, 
if $f \sim_\alpha g$ and $f$ is regular then $g$ is regular, because
an $\alpha$-conversion map $\phi$ is a bijection on sub-terms.
Using the generalmost notion of $\alpha$-conversion, instead,
$f = \lambda x^\N.\cond(x,f0)$ is $\alpha$-convertible to 
a non-regular term 
$\lambda x_1^\N.\cond(x_1,\lambda x_2^\N.\cond(x_2,(\lambda x_3^\N.\cond(...))0)0)$,
with infinitely many distinguished bound variables $x_1,x_2,\ldots$. 
...
From the typing rules we define the pre-proofs that a term is well-typed. 
...
Let $J$ be a judgment. 
We write $\Pi :: J$, and we say that $\Pi$ is a pre-proof of conclusion $J$ if:
\begin{enumerate}
\item 
...
There are non-sensical pre-proofs. For example, assume $t_n = t_{n+1}(0)$ is
...
\item
A pre-proof $\Pi$ is \emph{left-finite} if all leftmost paths $\pi$ in $\Pi$ 
