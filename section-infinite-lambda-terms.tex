...
If $t \sim_\alpha u$ and $t$ is regular then $u$ is regular, because
moving from $t$ to $u$ we replace some sub-term $v$ with some $\Phi:v \rightarrow w$,
so, at most, we have all sub-terms of $t$ and we add the subterms of $w$.
...
From the typing rules we define the \emph{pre-proofs} that a term is well-typed. 
