...
There is a unique infinite sub-term chain, which is $(t,\lambda x.t(x),t(x),t,\ldots)$. 
\end{Eg}
...
can be $\alpha$-equivalent to an infinite term 
with infinitely many distinct bound variables: in the example above we can
rename $\lambda x.t(x)$ to $\lambda x'.t(x')$, $\lambda x''.t(x'')$, \ldots.
...
We define the term $t[u/x]$ obtained by substituting the infinite term $u$ for the free variable $x$ in $t$ avoiding variable captures. In the case of regular
