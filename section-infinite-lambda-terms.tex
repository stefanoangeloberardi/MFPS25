...
\item
we write $t \sim u$ and we read \quotationMarks{$t$, $u$ are isomorphic terms}
...
\begin{definition}[Binders and $\alpha$-conversion]
...
the occurrence.
\item
We say that $t,u$ are $\alpha$-convertible and we write $t \sim_\alpha u$ if and only if there
is a tree isomorphism $\phi$ between $t$ and $u$ such that:
\begin{enumerate}
\item
$\phi$ preserves all labels $\not = \lambda x^T., x^T$
\item 
$\phi$ preserves all labels on a free variable occurrence, and the fact that the occurrence is free;
\item
$\phi$ sends each label $\lambda x^T.$ to some label $\lambda y^T.$, and each label $x^T$
on a bound variable to some label $z^T$ on a bound variable, in such a way that the
binder relation is preserved.
\end{enumerate}
\end{enumerate}
\end{definition}
...
%We write $\Stat = \Ctxt \times \Type$ for the set of all sequents.
...
free variable $x$ in $t$, by first replacing $t$ with some $\alpha$-convertible term $t'$
...
judgment, which we call its \quotationMarks{conclusion}.
...
Assume $\Pi=(T,\phi \times \psi)$ is a binary tree labeled on $\Stat \times \Rule$.
Assume $\Gamma \vdash t:A$ is judgment, $\nil$ is the root of $T$, and $l \conc \conc (i)$
is the child number $i$ of the node $l \in T$.
...
\item
  ...
  \item
    The children of $l$ in $T$ are in number of $p$ and are labeled 
	$
	...
	$
  \end{enumerate}
  Then the rule $r = \psi(l) \in \Rule$ has the form:
  \[
  \infer[r]{
    \Delta \prove u : B
  }{
...
finitely many rules in $\pi$ such that $\pi$ does \emph{not} 
...
For us, almost-left-finite proofs are the only making sense.
Indeed, we define the well-typed terms of $\LAMBDA$ 
