...
We say that a term is well-typed if it has a proof-tree satisfying some extra 
condition removing some non-sensical proofs.
...
\item
A path is \emph{almost-leftmost} if all nodes of $\pi$ but finitely many
are the left-child of the previous node of $\pi$.
...
for some $A$ and some context $\Gamma$ with $\FV(\Gamma) \supseteq \FV(t)$.
...
are almost-left finite,
then $\Pi:\Gamma \vdash t[u/x^U]:A$ for some almost-left finite $\Pi$.
\end{proposition}
