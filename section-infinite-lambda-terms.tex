...
$\alpha,\beta,\ldots$ of type variables, and for all types $A,B$ the type  $A \rightarrow B$.
...
$\{x^T, \ \lambda x^T., \ \ap, 0, \ \Succ, \ \cond\}$ such that:
\begin{enumerate}
\item 
a node labeled $x^T$ or $0$ is a leaf of $t$ (has \emph{no} children). 
\item
a node labeled $\lambda x^T.$ or $\Succ$ has a \emph{unique} 
child $u$. We write the node as $\lambda x^T.u$.
\item
...
we write $\ap(t,u)$ no matter if $u$ is a variable or not, but we split 
...
We will need the information in $\phi$ to state the Global Trace condition for terms
of $\LAMBDA$.
