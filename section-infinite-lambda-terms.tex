...
Binary trees are represented by set of lists on $\{1,2\}$.
...
Now we define a set $\LAMBDA$ of infinite tree notations: first the types of $\LAMBDA$, then the terms and the contexts of $\LAMBDA$.
...
Terms of $\LAMBDA$ are binary trees, with each node labeled by some operation
on $\lambda$-terms of arity $0,1,2$.
...
this relation defines a \emph{finite} graph with possibly cicles. Remark that 
...
Regular terms have finitely many distinct free variable and finitely many 
distinct bound variables, even if each variable can occur infinitely many times,
...
to $\lambda x'.t(x')$, $\lambda x''.t(x'')$, \ldots. 
...
free variable $x$ in $t$ avoiding variable captures. We can deduce the
usual equations for substitution, which we skip in this paper. In the case of regular
terms, we can renaming while preserving the finiteness of the graph.
...
\begin{lemma}[Substitution lemma]\label{lem:subst}
  $t[u/x][v/y]$,$ t[t/y][u[v/y]/x]$ are $\alpha$-equivalent if $x\not\in\FV(v)$. 
\end{lemma}
...
Remark that we introduced a unique \emph{term notation} for application: 
...
$x$, and $\apnotvar$ for $u$ not a variable. 
If, say, we replace $x^N$ with $\Succ(x^N)$ then we also replace the rule
$\apvar$ with $\apnotvar$. 
...
A rule is uniquely determined from its conclusion. We have the following 
derived rule $\weak$-rule for Weakening + Exchange:
if $\Gamma \vdash t:T$ and $\Gamma \subseteqsim \Gamma'$
then $\Gamma' \vdash t : T$. If we derive $\Gamma \vdash t:T$, 
then all contexts in the proof have prefix $\Gamma$, and the
rule $\weak$ can be derived by replacing everywhere $\Gamma$
by $\Gamma'$. This replacement preserves the correctness of the
$\var$-rules because we assumed $\Gamma \subseteqsim \Gamma'$.
...
Assume $r, r' \in \Rule$ have conclusion 
$\Gamma \vdash t:A$, $\Gamma' \vdash t':A'$ respectively.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The Almost-Left-Finite condition will ask
that all leftmost branches in any sub-proof are finite, say, that no infinite leftmost 
branch made of $\weak$-rules or of $\apnotvar$ exists. 
...
using almost-left-finite proofs.
...
We prove that the type assigned to a term by an Almost-Left-Finite proof is 
\emph{unique}: if two Almost-Left-Finite proofs $\Pi$, $\Pi'$ assign two types$A$, $A'$ to the same possibly infinite term $t$, then $\Pi = \Pi'$ and 
$A = A'$.
...
Some terms, those satisfying the global trace condition (to be introduced later) 
will denote a simple and broad subset of the total functionals.
...
n$ such that $(i_1, \ldots, i_h)$ has label $\cond$ and $i_{h+1} = 2$: i.e., the 
number of $\cond$ such that $l$ occurs in the right-hand-side of such $\cond$).
...
We abbreviate \quotationMarks{$0$-safe normal} 
with just \quotationMarks{safe normal}.
\end{enumerate}
\end{definition}
...
of $z$ because it is irrelevant. Then $u$ is $0$-safe-normal (safe normal for short), 
...
We will prove that for this taks we only
...
having length $(n+1)$ and terminating in $1$, independently from $n$.
\end{Eg}

There are meaningful 
terms without a normal form, but we can get as close as we want to a normal form
In fact, we will prove that for all $n$, 
...
obtained by taking the limit of the $n$-safe normal forms of the term.
