...
possibly infinite terms in $\LAMBDA$. If $t, u \in \LAMBDA$ then 
...
Regular terms have finitely many distinct variables, 
even if each variable can occur infinitely many times,
...
regular, yet is it $\alpha$-equivalent to $t$.
...
so in fact a type assignment $x^T:T$ is \emph{redundant} for our variables 
(not for our terms).
...
$\Gamma' = (x'_1:A'_1, \ldots, x'_n:A'_{n'})$ are context of $\LAMBDA$, then we write 
...
terms, we can rename while preserving the finiteness of the graph denotation
for the term.
...
\begin{lemma}[Substitution lemma]\label{lem:subst}
  $t[u/x][v/y]$ and $ t[v/y][u[v/y]/x]$ are $\alpha$-equivalent if $x\not\in\FV(v)$. 
\end{lemma}
...
$x$, because in this case we will also check whether the first
argument of $t$ is infinitely decreasing in any infinite computation.
If this is the case, then no infinite computation exists. 

If $u$ is not a variable,
then we cannot observe whether the argument of an application $t(u)$
is infinitely decreasing in any infinite computation. However we can write
this term in the form $(\lambda x.t(x))(u)$ for $x \not \in \FV(t)$. 
In this case $u$ is assigned to $x$ and we can observe whether
...
regularity of the term: the fact that we only have finitely many distinct subterms
and therefore we have a finite graph notation for the term.
 
Indeed, we use $\weak$-rule to
remove bound variables no more in use when whenever we unfold a fixed point 
definition. Without $\weak$-rule, we would be forced to introduce more and more
fresh names for bound variables in order to prevent variable capture.
Eventually we would add infinitely many fresh variable names, 
and this would destroy the regularity of the term.
...
then the leftmost branch uniquely  determines the type of the term and therefore it should be finite for the term ot make sense.
%12:54 03/06/2024
...
\emph{unique}: if two Almost-Left-Finite proofs $\Pi$, $\Pi'$ assign two types$A$, $A'$ to the same possibly infinite term $t$, then $A = A'$.
