...
\item
  ...
  by $A_1,A_2,\ldots,A_n \rightarrow B$. 
...
if and only if there is some label-preserving bijection between $t$ and $u$. 

We say that $t$ is a subterm of $u$ if $t$ is a sub-tree of $u$,
with labeling the restriction of the labeling of $u$ to the nodes of $t$.
An immediate subterm is an immediate sub-tree.
\end{definition}

We identify a term with its (possibly infinite) one-line denotation,
...
\begin{enumerate}
\item
$\phi$ preserves all labels except $\lambda x^T$ and $x^T$;
\item 
...
We write $\Stat$ for the set of all typing judgments.
...
\[
\Rule = 
\{\,r \in \Stat \cup \Stat^2 \cup \Stat^3 \mid \mbox{$r$ is an instance of some rule for $\LAMBDA$}\,\}.
\]
\end{definition}
...
There are non-sensical typing proofs. For example, assume $t_{n+1} = t_n(0)$ is
a term family indexed on $n \in \Nat$. Then, for any type $A$, we can define a proof family
$\Pi_n:\ \vdash t_n : \N^n \rightarrow A$. 
The assumptions of the conclusion of $\Pi_n$  are proved by
$\Pi_{n+1} :\ \vdash t_{n+1} : \N^{n+1} \rightarrow A$ 
and by $\Pi:\ \vdash 0:\N$. 
...
The one-step reduction relation $\reduces$
